...
/// ⚙️ EXPLORER LOGIC (FULL & FIXED)
/// Backend logic for the File Manager.
/// Handles File System, Navigation, Selection, Clipboard, and Process execution.
...
class ExplorerController extends ChangeNotifier {
  ...
  List<FileSystemEntity> _files = [];       // Files currently displayed
  List<FileSystemEntity> _allFiles = [];    // All files in current folder (for search)
  List<String> _drives = [];
  ...
  Future<void> renameEntity(String oldPath, String newName) async {
    try {
      final entity = FileSystemEntity.isDirectorySync(oldPath) 
          ? Directory(oldPath) : File(oldPath);
  ...
  /// Deletes selected items recursively.
  ...
  void search(String query) {
    ...
    if (query.isEmpty) {
      ...
      _files = _allFiles.where((entity) {
        final name = entity.uri.pathSegments.lastWhere((e) => e.isNotEmpty).toLowerCase();
  ...
  void copyToClipboard({bool isCut = false}) {
    ...
    _showStatus(isCut ? "Cut ${_clipboardFiles.length} items" : "Copied ${_clipboardFiles.length} items");
    notifyListeners();
  }
  ...
  // ---------------------------------------------------------------------------
  // 🚀 ACTIONS (OPEN, SHORTCUT, OPEN WITH)
  // ---------------------------------------------------------------------------

  /// Opens a file using Windows default application.
  /// FIXED: Uses `cmd /c start` to ensure it opens the file, not the explorer folder.
  void openFile(String path) {
    if (FileSystemEntity.isDirectorySync(path)) {
      navigateTo(path);
    } else {
      // 'start "" "path"' allows paths with spaces to open correctly
      Process.run('cmd', ['/c', 'start', '', path]);
    }
  }

  /// Opens the "Open With" dialog.
  void openWith(String path) {
    Process.run('rundll32.exe', ['shell32.dll,OpenAs_RunDLL', path]);
  }

  /// Creates a .bat shortcut because real .lnk creation requires FFI/C++.
  Future<void> createShortcut(String targetPath) async {
    try {
      final name = targetPath.split('\\').last;
      final batFile = File('$_currentPath\\Open $name.bat');
